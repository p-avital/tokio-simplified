...
pub mod tokio_simplified {
    ...
    pub struct AsyncWriter<SendType> {tx: RefCell<futures::sync::mpsc::Sender<SendType>>,}
    ...
    impl<SendType> AsyncWriter<SendType> {
        ...
        pub fn write(&self, frame: SendType) -> Result<futures::AsyncSink<SendType>, futures::sync::mpsc::SendError<SendType>> {
    ...
    impl<SendType, ReceiveType> AsyncReadWriter<SendType, ReceiveType>{
        ...
        pub fn new<Io, Codec>(sink: stream::SplitSink<Framed<Io, Codec>>, stream: stream::SplitStream<Framed<Io, Codec>>) -> AsyncReadWriter<<Codec as Encoder>::Item, <Codec as Decoder>::Item>
            where Codec: Decoder + Encoder + std::marker::Send + 'static,
                  ...
                  <Codec as Decoder>::Error: std::marker::Send
        {
            ...
            let subscribers = Arc::new(Mutex::new(Vec::<futures::sync::mpsc::Sender<<Codec as Decoder>::Item>>::new()));
            let stream_subscribers_reference = subscribers.clone();
            let stream_task = stream.for_each(move |frame: <Codec as Decoder>::Item|{
        ...
        pub fn on_receive<F>(&self, callback: F) where F: FnMut(ReceiveType)->Result<(),()> + std::marker::Send + 'static,
        ...
        pub fn get_writer(&self) -> AsyncWriter<SendType>{
            AsyncWriter {tx: RefCell::new(self.tx.clone())}
        }
    }
